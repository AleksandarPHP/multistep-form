  <div class="form-step" data-step="1" id="step-1">
    ...
    <div class="row g-4">
      ...
      <div class="col-lg-3 col-md-6">
        <div @class(['choice-card produktePosition', 'selected'  => $loop->first])>
          <div class="choice-image">
            @if($item->image)
            <img
          ...
          <div class="choice-text">
            <p>{{ $item->title }}</p>
            <input class="d-none" type="radio" data-price="{{ $item->price }}" name="product_position" id="product_position_{{ $item->id }}" value="{{$item->id}}" @checked($loop->first)>
          </div>
        </div>
      </div>
      @endforeach

...
class OptionItemController extends Controller
{
    ...
    {
        return view('cms.option-items.list',['id'=> $id]);
    }
    ...
    {       
        ...
        foreach($rows as $row) {
            ...
            $data[] = [
                ...
                '3' => '<a href="'.url('cms/option-items/'.$row->id.'/edit').'" class="action-edit"><i class="fa fa-edit"></i></a><a href="'.url('cms/option-items').'" class="action-delete confirmation" data-id="'.$row->id.'"><i class="fa fa-trash"></i><form id="delete-form'.$row->id.'" action="'.url('cms/option-items/'.$row->id).'" method="POST" style="display: none;">'.csrf_field().'<input type="hidden" name="_method" value="delete" /></form></a>',
            ];
        }
    ...
    {
        $options = Option::where('is_active', 1)->get();
        return view('cms.option-items.form', ['item' => new OptionItem(), 'editing' => false, 'options' => $options]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $image = $request->image;
        if($request->hasFile('image')) $image = Helper::saveImage($request->image, 'Item', $request->title, $image);
    ...
    {
        $options = Option::where('is_active', 1)->get();
        return view('cms.option-items.form', ['item' => OptionItem::findOrFail($id), 'editing' => true, 'options' => $options]);
    }

    public function update(Request $request, $id)
    {
        // dd($request->all());
        $item = OptionItem::findOrFail($id);
    ...
    {
        $user = OptionItem::findOrFail($id);    
        $user->delete();

        Cache::forget( 'otpion_item');
